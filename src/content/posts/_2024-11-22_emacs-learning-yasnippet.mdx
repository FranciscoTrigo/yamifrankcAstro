...
Doom emacs comes with it preinstalled, so I did not have to do anything about it. Please follow the instructions on [Github](https://github.com/joaotavora/yasnippet) to install it, there is a plethora of ways to do it.

Now let's start using it and add our first snippet. Push `M-x` (in emacs terms that would be Alt-x), this shoud open a buffer for you to run commands, we are going to do `yas-new-snipet`.
...
Emacs will automatically take you to `${1:function_name}`, where you can start writting your function name right away. Press `Tab` when you finish
and it will take you to `${2:args}`, and so on. This saves a lot of time!

## Ending

And that is it. Now you can go to some file and start typing the key that you set, a small completion buffer will pop up giving you the option to expand into your snippet!